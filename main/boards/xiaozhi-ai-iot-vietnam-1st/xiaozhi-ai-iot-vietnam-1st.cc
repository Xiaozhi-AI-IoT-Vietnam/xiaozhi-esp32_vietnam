...
static const nv3023_lcd_init_cmd_t lcd_init_cmds[] = {
    ...
    {0xE0, (const uint8_t[]){0x01, 0x03, 0x0d, 0x0e, 0x0e, 0x0c, 0x15, 0x19}, 8,
     0},
    {0xE3, (const uint8_t[]){0x1a, 0x16, 0x0C, 0x0f, 0x0e, 0x0d, 0x02, 0x01}, 8,
     0},
...
public:
  LogoLcdDisplay(esp_lcd_panel_io_handle_t panel_io,
...
private:
  ...
  void InitializePowerSaveTimer() {
    power_save_timer_ =
        new PowerSaveTimer(-1, SECONDS_TO_SLEEP_MODE, SECONDS_TO_SHUTDOWN);
  ...
  void InitializeButtons() {
    // Single click: Toggle Chat (talk to AI)
    ...
    // 6 clicks: Reset WiFi
    boot_button_.OnMultipleClick([this]() { ResetWifiConfiguration(); }, 6);

    volume_up_button_.OnClick([this]() {
      power_save_timer_->WakeUp();
      auto &app = Application::GetInstance();
      
      // If Intercom UI is visible, use for navigation
    ...
    // Double-press volume up: Select in Intercom UI
    ...
    volume_up_button_.OnLongPress([this]() {
      ...
      // If Intercom UI is visible, cancel it
      if (app.IsIntercomContactsVisible()) {
    ...
    // Double-press volume down: Select in Intercom UI  
    ...
    volume_down_button_.OnLongPress([this]() {
      ...
      // Toggle AEC mode (Realtime vs AutoStop listening)
  ...
  void InitializeNv3023Display() {
    ...
    esp_lcd_panel_io_spi_config_t io_config =
        NV3023_PANEL_IO_SPI_CONFIG(DISPLAY_CS, DISPLAY_DC, NULL, NULL);
    ...
    ESP_ERROR_CHECK(
        esp_lcd_new_panel_nv3023(panel_io_, &panel_config, &panel_));
    ...
    display_ = new LogoLcdDisplay(
        panel_io_, panel_, DISPLAY_WIDTH, DISPLAY_HEIGHT, DISPLAY_OFFSET_X,
...
public:
  ...
  XINGZHI_CUBE_1_83TFT_WIFI_1ST()
      : boot_button_(BOOT_BUTTON_GPIO, false, 2000, 0, false),
  ...
  virtual SdCard *GetSdCard() override {
    ...
    static SdMMC sdmmc(CARD_SDMMC_CLK_GPIO, CARD_SDMMC_CMD_GPIO,
                       CARD_SDMMC_D0_GPIO, CARD_SDMMC_D1_GPIO,
                       CARD_SDMMC_D2_GPIO, CARD_SDMMC_D3_GPIO);
#else
  ...
  virtual SdCard *GetSdCard() override {
    static SdSPI sdspi(CARD_SPI_MISO_GPIO, CARD_SPI_MOSI_GPIO,
                       CARD_SPI_SCLK_GPIO, CARD_SPI_CS_GPIO);
    return &sdspi;
  }
#endif
};
